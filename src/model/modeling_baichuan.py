...
from transformers import PreTrainedModel, TextIteratorStreamer, TextStreamer
...
try:
    ...
    xops = None
    warnings.warn('If you want train model, please install xformers', UserWarning)
...
class BaiChuanChatBot(BaiChuanForCausalLM):
    def __init__(self, config):
        """
    ...
    def prepare_inputs(
        ...
        Args:
            ...
            prompt:
            history:
            add_prefix4single: 没有历史对话记录时，对当前语句是否添加前缀
            query_prefix:
            answer_prefix:
        ...
        cur_input = ''.join((query_prefix, query, '\n', answer_prefix)) if (add_prefix4single or len(history) > 0) else query
        input_lst.append(cur_input)
        input_lst = ([prompt, '\n'] + input_lst) if prompt and prompt[-1] != '\n' else ([prompt] + input_lst)
        raw_input = ''.join(input_lst)
        input_ids = tokenizer([raw_input], return_attention_mask=False, return_tensors='pt')['input_ids'].to(self.device)
