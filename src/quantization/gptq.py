...
@datetime   : 2023/6/16 下午8:49
@description: Modified based ChatGLM-6b: <https://huggingface.co/THUDM/chatglm-6b/tree/main> quantization.py
"""
...
try:
    ...
    raise ValueError('While use `gptq` method to quantize model, you firstly should install `cpm_kernels`')
...
def gpt_quantize(model, weight_bit_width, empty_init=False, **kwargs):
    """Replace fp16 linear with quantized linear"""

    if weight_bit_width is not None:
        if not isinstance(weight_bit_width, (float, int)): raise ValueError('The bit width should be an integer, 4 or 8')
        bits = int(weight_bit_width)
        ...
        for layer in model.model.layers:
            layer.self_attn.W_pack = QuantizedLinear(
                ...
                weight_tensor=layer.self_attn.W_pack.weight.to(torch.cuda.current_device()),
                bias_tensor=layer.self_attn.W_pack.bias,
                in_features=layer.self_attn.W_pack.in_features,
                out_features=layer.self_attn.W_pack.out_features,
                bias=True,
                dtype=torch.half,
                device=layer.self_attn.W_pack.weight.device,
        ...
        logger.warning('The model was not quantized.')
